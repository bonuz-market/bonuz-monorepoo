...
const deploy: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  log('----------------------------------------------------')
  log('Deploying BonuzSocialId ...')

  const waitBlockConfirmations = developmentChains.includes(network.name)
    ? 1
    : VERIFICATION_BLOCK_CONFIRMATIONS

  const contract = await deploy('BonuzSocialId', {
    from: deployer.address,
    args: [[], [...SUPPORTED_PLATFORMS]],
    log: true,
...
deploy.id = 'deploy_BonuzSocialId' // id required to prevent re-execution
deploy.tags = ['all', 'BonuzSocialId']
