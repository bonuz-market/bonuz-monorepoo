...
import { AddNewChallengesList } from '@/components/Partner/AddNewChallengesList'
...
const CreatePartnerEvent = () => {
  ...
  const [createEventMutation, { data: eventData, loading, error }] =
  ...
  const { data } = useQuery(GET_NEW_PARTNERS_QUERY, {
    context: {
      headers: {
        authorization: `Bearer ${token}`,
      },
  ...
  const onSubmit = async (data: CreatePartnerFormValues) => {
    ...
    const variables = {
      ...
      description: data.description,
      agenda: data.agenda,
      image: data.image.doc.id.toString(),
      banner: data.banner.doc.id.toString(),
      link: data.link,
