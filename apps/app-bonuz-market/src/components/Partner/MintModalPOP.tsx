...
import CanvasTicketPOP from './CanvasTicketPOP';
...
export const MintModalPOP = ({
  ...
  imageData: ImageData
  user: User
  handleMint: (base64Image: string, expiryDate: number) => Promise<void>
  isLoading: boolean
  ...
  const handleMint = useCallback(async () => {
    ...
    try {
      ...
      await handleMintCallback(uri, expiryDateMs);
      setOpen(false)
    } catch (error) {
      console.log(error);
      setIsLoading(false);
  ...
  }, [expiryDate, handleMintCallback, setIsLoading, setOpen]);
                  ...
                  <div className="mt-3 flex w-full flex-col gap-4 text-center sm:mt-0 sm:text-left lg:p-4">
                    ...
                    <div>
                      ...
                      <div className='flex items-center justify-center mt-10 gap-10'>
                        <div>
                          <label className='text-black dark:text-white'>Background Color:</label>
                          <input
                            type="color"
...
const ImageEditor = memo<ImageEditorProps>(
  ({ templateImage, logo, text, setBase64Image }) => {
    ...
    }, [templateImage, text.courseTitle, logo.src, logo.width, logo.height, setBase64Image]);
