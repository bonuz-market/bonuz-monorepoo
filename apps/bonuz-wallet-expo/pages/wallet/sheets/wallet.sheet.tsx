...
import {
  getActivityDataByWalletAddress,
  getNftDataByWalletAddress,
  getTokenDataByWalletAddress,
  getTotalBalanceByWallet,
} from '@/services/backend/wallets.service';
...
export const WalletSheet = forwardRef<BottomSheetModal, WalletSheetProps>(
  (
    ...
    const { data } = useQuery({
      ...
      queryFn: ({ queryKey }) => {
        switch (queryKey[2]) {
          case 'Crypto': {
            setLoading(true);
            return getTokenDataByWalletAddress(queryKey[0], queryKey[1], setLoading);
          }
          case 'NFTs': {
            setLoading(true);
            return getNftDataByWalletAddress(queryKey[0], queryKey[1], setLoading);
          }
          case 'Activity': {
            setLoading(true);
            return getActivityDataByWalletAddress(queryKey[0], queryKey[1], setLoading);
          }
        }
      },
    });
      ...
      queryFn: ({ queryKey }) => {
        for (const networkType of networkTypes) {
          if (swapNetwork === networkType.network) {
            return getTokenDataByChainId(
              Number(networkType.chainId),
              setSwapToken,
              setSwapDesToken,
              swapTokenType,
            );
    ...
    const handleNetwork = (networkString: string) => {
      if (flag === 'swap') setSwapNetwork(networkString);
      else setDestinationNetwork(networkString);
    };

    const handleFiatOnRampClick = async () => {
      const secret =
        Constants.expoConfig?.extra?.mercuryo.secret ?? '<backendUrl must be set in app.json>';
      const widgetId = Constants.expoConfig?.extra?.mercuryo.widgetId;
      ...
      await Linking.openURL(
        `https://exchange.mercuryo.io?widget_id=${widgetId}&address=${wallet.address}&signature=${signature}`,
      );
    };
    return (
          ...
          <ImageBackground
            ...
            <View style={tw`w-full bg-transparent p-5 h-full`}>
              <View style={tw`bg-transparent`}>
                ...
                <Text style={tw`font-medium text-[14px] text-white`}>
        ...
        {isNotEmpty(auth) && isNotEmpty(user) ? (
          <View style={tw`flex-1 bg-transparent mt-[-130]`}>
            ...
            <ScrollView style={tw`bg-transparent flex-1`}>
              {value === 'Crypto' && <TokenInfoSection value={data} loadingStatus={loading} />}
              {value === 'NFTs' && <NftInfoSection value={data} loadingStatus={loading} />}
              {value === 'Activity' && <ActivityInfoSection value={data} loadingStatus={loading} />}
            </ScrollView>
          </View>
        ) : (
          <WalletUnConnected />
                ...
                <ScrollView style={tw`bg-transparent flex-1`}>
                    ...
                    <View>
                      <View style={tw`flex flex-row mx-10 justify-between my-5`}>
                        <TouchableOpacity onPress={() => setOption('receive')}>
                          <Text
                            style={[
                              tw`text-[20px] text-white p-2`,
                              option === 'receive' && tw`border-[#EC6640] border-b-2`,
                            ]}>
                            Receive
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setOption('buyType')}>
                          <Text
                            style={[
                              ...
                              option === 'buyType' && tw`border-[#EC6640] border-b-2`,
                            ]}>
                            Buy
                          </Text>
                        </TouchableOpacity>
                      ...
                      {option === 'receive' ? (
                        ...
                        <View style={tw`flex-1`}>
                          <Pressable
                            style={tw`flex-row items-center justify-between bg-[#5137B1] rounded-3xl p-5 m-5`}
                            onPress={handleFiatOnRampClick}>
                  ...
                  {currentSection === 'swap' && (
                    <SwapComponent
                      ...
                      handleDismissModalPress={handleDismissSwapModalPress}
                      handleTokenSection={handleTokenSection}
                      swapToken={swapToken}
                      swapDesToken={swapDesToken}
                      setSwapTokenType={setSwapTokenType}
                    />
                  )}
                  <BottomSheetModal
                          ...
                          tokens.map((value: TokenProps, index: number) => (
                            <TouchableOpacity
                              ...
                              <Image
                                ...
                                source={{ uri: value.logoURI }}
                              />
                              <Text style={tw`text-[16px] font-medium text-white ml-2`}>
                                {value.name}
                              </Text>
                            </TouchableOpacity>
                          ))}
